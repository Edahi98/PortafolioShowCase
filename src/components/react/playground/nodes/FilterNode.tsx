...
export function FilterNode({ data }: NodeProps<FilterData>) {
	...
	return (
		<div className="node-base border-purple-500/50 shadow-purple-500/20" style={{ minWidth: 196 }}>
			...
			<div className="mt-3 flex flex-col gap-2">
				<div className="flex flex-col gap-0.5">
					<label className="font-mono text-[9px] uppercase text-slate-500">Búsqueda fuzzy</label>
					<input
						type="text"
						value={data.query}
				...
				<div className="grid grid-cols-2 gap-1.5">
					<div className="flex flex-col gap-0.5">
						<label className="font-mono text-[9px] uppercase text-slate-500">Categoría</label>
						<select
							value={data.categoryFilter}
							onChange={(e) => data.onCategoryChange(e.target.value as Category | 'All')}
					...
					<div className="flex flex-col gap-0.5">
						<label className="font-mono text-[9px] uppercase text-slate-500">Nivel</label>
						<select
							value={data.difficultyFilter}
							onChange={(e) => data.onDifficultyChange(e.target.value as Difficulty | 'All')}
							className="nodrag rounded-md border border-purple-500/30 bg-slate-800/80 px-1.5 py-1 font-mono text-[10px] text-slate-100 focus:border-purple-400/60 focus:outline-none"
						>
			...
			<div className="mt-3">
				<div className="flex justify-between mb-1">
					<span className="font-mono text-[10px] text-purple-300 font-bold">{data.count} resultados</span>
